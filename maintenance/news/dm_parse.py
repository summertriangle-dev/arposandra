...
class DMWalkState(object):
    ...
    def ingest_text_internal(self, t_i):
        if len(self.root) == 0:
            self.root.text = "".join((self.root.text or "", t_i))
        else:
            self.root[-1].tail = "".join((self.root[-1].tail or "", t_i))
    ...
    def ingest_element_internal(self, element):
        if element.tag in {"color", "align", "size", "b"}:
            ...
            SubElement(
                sub,
                "a",
                {"href": make_link_inert(element.get("link_text")), "class": "btn btn-primary",},
            ).text = element.get("button_text")
        elif element.tag == "button":
            sub = SubElement(self.root, "div", {"class": "button-wrapper"})
            SubElement(
                sub, "button", {"class": "btn btn-primary", "disabled": "disabled"}
            ).text = element.get("button_text")
    ...
    def get_html(self):
        return tostring(self.commit(), encoding="utf-8").decode("utf8")
